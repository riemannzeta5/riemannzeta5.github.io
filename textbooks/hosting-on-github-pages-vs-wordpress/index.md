...
(WordPress' reason for doing this is a very good one: it wants to make sure that people don't accidentally overwrite content when reuploading. If you host a PDF on WordPress named `file.pdf` and then re-upload it with different content, the new version would automatically be renamed as `file (1).pdf`. However, because I *want* to update these files continuously, in my case that's not what I want.)

As a result, I've settled on hosting textbooks and longer expository series here, on GitHub Pages.
